...
void vTaskConsole(void *pvParameters) {
    while(1) {
        // Envia apenas uma linha JSON pura contendo todas as variáveis da PCB
        // O \n no final é obrigatório para o Python saber onde a linha termina
        printf("{\"pitch\":%.2f,\"roll\":%.2f,\"joy_x\":%d,\"joy_y\":%d,\"btn\":%d,\"srv_x\":%d,\"srv_y\":%d}\n", 
               g_pitch, g_roll, g_joy_x, g_joy_y, g_btn, g_servo_x, g_servo_y);
        
        // Reduzimos o tempo de espera de 500ms para 100ms
        // Isso envia 10 leituras por segundo, deixando o Grafana em tempo real
